...
function loadAllQuestions(){
    ...
    // get the container that will hold all questions
    const questionsContainer = document.getElementById("questionsContainer");

    // create a div for each question with its options
    questions.forEach((q, index) => {
        const eachQuestionCard = document.createElement("div");
        eachQuestionCard.classList.add("card");
        eachQuestionCard.innerHTML = `
        <h2 class="question">${index + 1}. ${q.question}</h2>
        <ul class="choicesContainer">
        ${q.options.map((option, i) => `<li class="option" onclick="checkAnswer(this, ${q.correct})">${option}</li>`).join('')}
        </ul>`;
    questionsContainer.appendChild(eachQuestionCard);
    });
}
